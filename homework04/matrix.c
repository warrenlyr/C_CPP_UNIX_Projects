...
void matrix_transpose(int** dst, int** src, int rows, int cols)
{
    ...
    for(int i = 0; i < rows; i ++){
        for(int j = 0; j < cols; j ++){
            dst[j][i] = src[i][j]; //Just as description, src[i][j] -> dst[j][i]
        }
    }
}
              ...
              int* color, int* distance)
{
    ...
    while(!done) {
        ...
        for(int i = 0; i < rows; i ++){
            int result = 0;
            for(int j = 0; j < cols; j ++){
                result += src[j] * int_array[i][j];
            }
            dst[i] = result;
        }
        
        //Check for if visited,
        //if the node is visited, delete it from vec
        for(int i = 0; i < rows; i ++){
            if(distance[i] > 0){
                dst[i] = 0;
            }
        ...
        //Change the input vec
        //to the new vec(dst(res))
        for(int i = 0; i < rows; i ++){
            src[i] = dst[i];
        }

...
void construct_adj_list(int** adj_mat, int rows, int cols, adj_node_t*** list)
{
    ...
    for(int i = 0; i < rows; i ++){
        for(int j = 0; j < cols; j ++){
            //Each index, if found non-zero value
            //Add to list using add_node()
            if(adj_mat[i][j] == 1){
         ...
         int* color, int* distance, int* parent)
{
    ...
    adj_node_t *s_node = create_node(source);
    add_node(queue, 0, s_node);
    ...
    while(queue[0] != NULL){
        int thisParent = remove_node(queue); //Dequeue the first one, get value
        adj_node_t *thisNode = list[thisParent]; //Find this parent in the list
        while(thisNode != NULL){ //Read through the parent's row until reach the end
            if(color[(thisNode->vid)] == 0){ //Set these values if it's not visited
                color[(thisNode->vid)] = 1; //Color -> visited, not dequeued
                distance[(thisNode->vid)] = distance[thisParent] + 1; //Distance ->parent's distance + 1
                parent[(thisNode->vid)] = thisParent; //Set parent as this parent
                add_node(queue, 0, create_node(thisNode->vid)); //Add to queue
            }
            thisNode = thisNode->next;
        }
        color[thisParent] = 2; //This parent is visited
        free(thisNode);
    }
    free_adj_list(queue, 0);

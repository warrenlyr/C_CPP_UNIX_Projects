...
{
    //Create array for pointing to it in the end
    unsigned int *csr_row_ptr_ = malloc(sizeof(unsigned int) * (m + 1));
    unsigned int *csr_col_ind_ = malloc(sizeof(unsigned int) * nnz);
    double *csr_vals_ 
    = malloc(sizeof(double) * nnz);
    //Check
    if(csr_row_ptr_ == NULL || csr_col_ind_ == NULL || csr_vals_ == NULL){
        free(csr_row_ptr_);
        free(csr_col_ind_);
        free(csr_vals_);
    ...
    //Logic here is loop for [index of row_ptr] times, (m + 1) times
    int count = 0;
    for(int i = 0; i < m; i ++){    //In each row we are searching, go through the whole row_ind arr
        for(int j = 0; j < nnz; j ++){  //If find the row_ind = the row we are searching for
            if(row_ind[j] == (i + 1)){
                csr_col_ind_[count] = col_ind[j];   //Store the val and col
                csr_vals_[count] = val[j];
                count ++;   //Count is for count the num we have already store in csr arr
            }
        }
        
        if(i == 0){ //In my logic, I'm always set the end index of current arr
            csr_row_ptr_[i] = 1;    //So the first index have to set the start index and the end index
            csr_row_ptr_[i + 1] = 1 + count;
        }
        else{
            csr_row_ptr_[i + 1] = 1 + count;
        }
    ...
    if(count != nnz){   //Check for if my count = nnz, if not, somwthing wrong
        fprintf(stderr, "NNZ error!\n");
        free(csr_row_ptr_);
        free(csr_col_ind_);
        free(csr_vals_);
        exit(EXIT_FAILURE);
    ...
    *csr_col_ind = csr_col_ind_;    //Point these to the arr I created
    *csr_row_ptr = csr_row_ptr_;
    *csr_vals = csr_vals_;
    /*
    printf("\nRow index: ");
    for(int i = 0; i < (m + 1); i ++){
        printf("%u ", csr_row_ptr_[i]);
    }
    printf("\nCol index: ");
    for(int i = 0; i < nnz; i ++){
        printf("%u ", csr_col_ind_[i]);
    }
    printf("\nVal index: ");
    for(int i = 0; i < nnz; i ++){
        printf("%f ", csr_vals_[i]);
    }*/
...
{
    for(int i = 0; i < m; i ++){    //Initialize the value to 0 first, in case something go wrong
        res[i] = 0;
    }

    for(int i = 0; i < m; i ++){    //Easy logic here, multiple the matrix with val
        for(int j = csr_row_ptr[i]; j < csr_row_ptr[i + 1]; j ++){
            res[i] += vector_x[csr_col_ind[j - 1] - 1] * (double)csr_vals[j - 1];
        }
    }

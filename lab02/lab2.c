...
capitalize_first_col(char* in_str, int row, int col, int rows, int cols)
{
  // TODO: add your code for part (2) here
    if(col == 0){
        for(int i = 0; i < strlen(in_str); i ++){    //From the first letter to the last
            in_str[i] = toupper(in_str[i]); //To upper case
        }
    }
             ...
             int rows, int cols)
{
  // TODO: add your code for part (4) here
    for(int i = 0; i < cols; i ++){ // Start at col 1
        for(int j = 0; j < rows; j ++){ // In each col, continue update row
